...
<img src="./img/ch5-进程切换.png" width="700">
...
可以将管道看成一个有一定大小的字节队列。它的读写两端，分别分配给两个文件描符。使用系统调用`sys_pipe`来创建管道。

<img src="./img/ch7-管道.png" width="300">

重定向则依赖系统调用`sys_dup`。它能分配新的文件描述符，指向另一个文件描述符所指的文件。这样，它们就指向了相同的文件。

<img src="./img/ch7-重定向.png" width="500">

结合系统调用`sys_pipe`和`sys_dup`，就可以实现命令行的管道功能，将前一个命令的输出，作为后一个命令的输入。

<img src="./img/ch7-管道命令.png" width="750">
...
- `sys_kill`：向某个进程发送信号
- `sys_sigaction`：为当前进程注册信号处理函数
- `sys_sigprocmask`：设置当前进程的全局信号掩码
- `sys_sigreturn`：通知内核，退出进程的信号处理程序，恢复到原先的执行流
...
同步互斥的原语，如互斥锁、条件变量、信号量等，一般是基于硬件的原子操作实现的。硬件会提供原子指令，如`load-reserved`、`store-conditional`等，来保证多线程间的操作是原子的。不过，这些同步互斥的实现方式很多，有一些可以在用户态实现。

### 9. I/O设备

I/O设备是计算机系统的重要组成部分，它们负责与外部世界进行数据交换。常见的I/O设备有键盘、鼠标、显示器、硬盘、网卡等。

操作系统若想跟I/O设备通信，就需要驱动程序（Driver）作为桥梁。通过驱动程序，操作系统可以向I/O设备发送命令，读取数据，反过来I/O设备可以向操作系统发送中断，通知操作系统有数据到来。驱动程序隐藏了I/O设备的细节，让操作系统能更方便地进行管理。对每个I/O外设，都需要一个对应的驱动程序。

I/O设备有自己的寄存器，可以用来接收命令或传递数据等。CPU访问I/O设备的方式主要有两种：

- I/O端口：I/O设备有独立的地址空间，与内存地址空间分离。CPU通过特殊的指令来访问I/O设备。
- 内存映射（MMIO）：I/O设备与内存共享地址空间，CPU通过读写内存的方式来访问I/O设备，只需要使用常规的内存读写指令。

CPU发送I/O请求后，I/O设备需要一定时间来完成任务。任务完成后，主要有两种方式来通知CPU：

- 轮询：CPU不断查询I/O设备的状态，直到任务完成。
- 中断：I/O设备完成任务后，向CPU发送中断信号。CPU收到中断信号后，暂停当前任务，处理中断请求。

中断只适合CPU和外设的数据处理能力相差很大的情况。如果外设本身速度（比如高速网卡的速率可以达到1000Gbps）就很快，那使用中断的额外开销（中断处理、中断上下文切换、进程上下文切换等）反而超过等待时间。此时不如采用轮询的方式。

但每次中断能传输的数据是有限的。如果在短期里传输大量数据，那就很慢。这时可以通过DMA（Direct Memory Access）技术，让I/O设备直接将数据传输到内存中。传输过程中，CPU不需要参与。只需要在传输完成后，让I/O设备发送中断通知CPU即可。

在虚拟机的语境下，虚拟机（guest）需要通过虚拟机管理软件（如Qemu/Hypervisor），才能访问宿主机（Host）的I/O设备。虚拟机管理软件需要模拟I/O设备（称为virtio），直接供虚拟机的驱动程序交互。这样，就能屏蔽虚拟机和宿主机的差异，让虚拟机能够透明地访问宿主机的I/O设备。

//! 虚拟文件系统（Virtual filesystem），屏蔽了具体的文件系统的实现细节。
//! 这是文件系统使用者直接使用的结构
...
impl Inode {
    ...
    // 如果self是类型为目录的inode，则查找名为name的inode项；否则报错。
    ...
    // 增加inode的大小，如果新的大小比原来的大，则分配新的数据块
    fn increase_size(
        &self,
        new_size: u32,
    ...
    // 若当前inode是目录，则创建一个名为name的文件。如果创建成功，则返回该文件的inode；
